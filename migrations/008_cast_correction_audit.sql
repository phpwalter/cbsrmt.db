...
CREATE TABLE IF NOT EXISTS import.cast_deletion_audit (
    deletion_id         bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    deletion_key        text NOT NULL UNIQUE CHECK (btrim(deletion_key) <> ''),
    person_id           integer NOT NULL CHECK (person_id > 0),
...
CREATE INDEX IF NOT EXISTS cast_deletion_audit_person_idx
    ON import.cast_deletion_audit(person_id, applied_at DESC);

COMMENT ON TABLE import.cast_deletion_audit IS
    'Append-only audit history of cast records intentionally removed after relationship validation.';
