...
def name_index(*collections: list[dict[str, Any]]) -> dict[str, set[str]]:
    ...
    for collection in collections:
        for person in collection:
            ...
            candidates = {full_name(person)}
            if first and last:
                candidates.add(f"{first} {last}")
                if middle:
                    candidates.add(f"{first} {middle[:1]} {last}")
                    candidates.add(f"{first} {middle[:1]}. {last}")
            for candidate in candidates:
                if norm(candidate):
                    aliases[norm(candidate)].add(cid)
    return aliases
    ...
    for episode in episodes:
        ...
        for part in split_writer_text(raw):
            ...
            if len(ids) == 1:
                ...
                resolved.append(int(WRITER_ALIASES[key]))
            elif len(ids) > 1:
