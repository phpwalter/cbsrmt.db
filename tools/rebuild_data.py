...
def generate_sql(genres,cast,episodes,appear,episode_writers,adaptations) -> str:
    ...
    lines += ["CREATE TABLE `episodes` (","  `episode_id` int unsigned NOT NULL,","  `episode_date` date NOT NULL,","  `episode_name` varchar(150) NOT NULL,","  `episode_plot` text NOT NULL,","  `genre_id` tinyint unsigned NOT NULL,","  `repeat_of_episode_id` int unsigned DEFAULT NULL,","  `otrw_id` int unsigned DEFAULT NULL,","  PRIMARY KEY (`episode_id`),","  KEY `idx_episodes_date` (`episode_date`),","  KEY `idx_episodes_name` (`episode_name`),","  KEY `idx_episodes_genre` (`genre_id`),","  KEY `idx_episodes_repeat_of` (`repeat_of_episode_id`),","  KEY `idx_episodes_otrw_id` (`otrw_id`),","  CONSTRAINT `fk_episodes_genre` FOREIGN KEY (`genre_id`) REFERENCES `genre` (`genre_id`) ON UPDATE CASCADE ON DELETE RESTRICT,","  CONSTRAINT `fk_episodes_repeat_of` FOREIGN KEY (`repeat_of_episode_id`) REFERENCES `episodes` (`episode_id`) ON UPDATE CASCADE ON DELETE SET NULL",") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;",""]
    ...
    for row in episodes: lines.append("INSERT INTO `episodes` (`episode_id`,`episode_date`,`episode_name`,`episode_plot`,`genre_id`,`repeat_of_episode_id`,`otrw_id`) VALUES ("+f"{int(row['episode_id'])},{sql_date(row['episode_date'])},{sql_string(row['episode_name'])},{sql_string(row['episode_plot'])},{int(row['genre_id'])},{sql_int_or_null(row.get('repeat_of_episode_id'))},{sql_int_or_null(row.get('otrw_id'))});")
